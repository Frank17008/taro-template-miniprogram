...
import { deleteMinIoImg, saveImg, deleteImg, UPLOAD_URL } from '@/service/common';
...
const UploadImage = ({ onChange, fileList, extra, disabled = false }: IUploaderImage) => {
  ...
  const beforeDelete: any = async (_file) => {
    if (extra?.mode === 'edit') {
      ...
      const deleteIndex = files?.findIndex((item) => _file?.name.indexOf(item.name) > -1);
